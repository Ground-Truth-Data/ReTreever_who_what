...
<style>
	/* Keep in sync with DOG_W_MIN/VW/MAX in GrassTufts.svelte (getComputedStyle read-back locks the renderer). */
	:global(:root) {
	...
	.mascot-run {
		...
		/* 0.55s = natural trot; widen --dog-w to make him linger, don't slow this. */
		--dog-leg: 0.55s;
	...
	/* Only X: rotate precedes transform, so translateX carries the tilt already;
	   end must clear --dog-w or he's parked on-screen for good. */
